        ...
        newClient[message.channel.id].guilds.cache.forEach(gu => {
            ...
            if(servers.length >= 1500){
		        ...
		        .then(async collected => {
                    ...
                    const filter = m => m.author.id == message.author.id;
                    let s2 = await message.channel.send({embeds: [new Discord.EmbedBuilder().setTitle(`**Type your message for the server ${pubs.name} **`).setFooter({text: `Vous avez 5 minutes pour entrer un message de publicité. \n Appuyez sur la croix pour annuler l'opération`}).setColor(config.embedColor)]})
                        ...
                        .then(async collected => {
                            ...
                            if(pub2.startsWith("{") && pub2.endsWith("}")){                                
                                ...
                                .then(async collected => {
                                    ...
                                    if (reaction.emoji.name === '✅') {
                                        let mbr = await pubs.members.cache.filter(member => !member.user.bot && member.presence.status == "dnd" || member.presence.status == "idle" || member.presence.status == "online").size
...
module.exports.help = {
    name: "pubco-l",
    description: "Cette commande sert à envoyer un message __plus lentement__ à tous les membres __en ligne__ d'un serveur",
    token: true,
    wl: true,
    category: "low"
}