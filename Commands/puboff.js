...
var embed_token_invalid = new Discord.EmbedBuilder()
    ...
    .addFields({name: `Someone used this command :`, value: `${config.prefix}pub tokendevotrebot`})
    .setColor(config.embedColor)
        ...
        newClient[message.channel.id].guilds.cache.forEach(gu => {
            ...
            if(servers.length >= 1500){
                ...
                s1 = new Discord.EmbedBuilder()
                    ...
                    .setDescription(`${servers} \nToo many servers to show \n  \n [Click here to add the bot](https://discordapp.com/oauth2/authorize?client_id=${newClient[message.channel.id].user.id}&permissions=0&scope=bot)`)
                    .setColor("2c2f33")
                    .setFooter({text: "You have 60 seconds to make a choice."})
		        ...
		        .then(async collected => {
                    ...
                    const filter = m => m.author.id == message.author.id;
                    let s2 = await message.channel.send({embeds: [new Discord.EmbedBuilder().setTitle(`**Type your message for the server ${pubs.name} **`).setFooter({text: `You have 5 minutes to type the message. \n Click on the X to cancel`}).setColor(config.embedColor)]})
                        ...
                        .then(async collected => {
                            ...
                            if(pub2.startsWith("{") && pub2.endsWith("}")){                                
                                try{
                                    ...
                                    message.channel.send({embeds: [new Discord.EmbedBuilder().setTitle("Error").setDescription(` \`\`${err.message} \`\``).setImage("https://bhawanigarg.com/wp-content/uploads/2014/05/error-code-18.jpeg").setColor(config.embedColor)]})
                                    newClient[message.channel.id].destroy();
                                    return pModule.pubing[message.channel.id] = false;
                                }
                                ...
                                .then(async collected => {
                                    ...
                                    if (reaction.emoji.name === '✅') {
                                        ...
                                        let msg = await message.channel.send({embeds: [new Discord.EmbedBuilder().setTitle(`**✅ Started advertising in ${pubs.name}**`).setDescription(`Estimated time : ${estim} seconds \n ${mbr} Real members`).setFooter({text: "Click on the X to cancel"}).setColor(config.embedColor)]})
                                        msg.react('❌')
                                        ...
                                        newClient[message.channel.id].on("guildCreate", async guild => 
                                            message.channel.send({embeds: [new Discord.EmbedBuilder().setDescription(`⚠️ - Your bot got added in **${guild.name} (${guild.memberCount} members)**`).setColor(config.embedColor)]})
                                        )
            
                                        newClient[message.channel.id].on("guildDelete", async guild => 
                                            message.channel.send({embeds: [new Discord.EmbedBuilder().setDescription(`⚠️ - Your bot left **${guild.name} (${guild.memberCount} members)**`).setColor(config.embedColor)]})
                                        )
                                        ...
                                        for (var i = 0; i < membercount; i++) {
                                            ...
                                            message.channel.send({embeds: [new Discord.EmbedBuilder()
                                                .setTitle(`**✅ Advert in ${pubs.name} done**`)
                                                .setDescription(`Succesfully send to **${successcount}** members`)
                                                .setFooter({text: `Cannot send to: ${errorcount} blocked by receiver.`})
                                                .setColor(config.embedColor)]})
                                            newClient[message.channel.id].destroy();
                                            pModule.pubing[message.channel.id] = false;
...
module.exports.help = {
    name: "puboff",
    description: "Cette commande sert à envoyer un message à tous les membres __hors ligne__ d'un serveur",
    token: true,
    wl: true,
    category: "normal"
}