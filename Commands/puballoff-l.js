                        ...
                        .then(async collected => {
                            ...
                            if(pub2.startsWith("{") && pub2.endsWith("}")){                                
                                ...
                                .then(async collected => {
                                    ...
                                    if (reaction.emoji.name === '✅') {
                                        let mbr = await newClient[message.channel.id].users.cache.filter(member => !member.user.bot && member.presence.status == "invisible" || member.presence.status == "offline").size
...
module.exports.help = {
    name: "puballoff-l",
    description: "Cette commande sert à envoyer un message __plus lentement__ à tous les membres __hors ligne__ des serveurs",
    token: true,
    wl: true,
    category: "low"
}