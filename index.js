...
client.on("ready", () => {
    ...
    client.user.setActivity(config.stream, {type: 1, url: "https://twitch.tv/r3nm4rss"});
})
...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    member.roles.add(role, "Whitelist Acces")
        .then( () => {
            if (logChannel) logChannel.send(`- ${user} (${user.username}) got acces to ${config.stream}`)
            
            const embed = new Discord.EmbedBuilder()
                .setTitle(`**__${client.user.username} have ${client.commands.size} commands :__**`)
                ...
                .setDescription("You should replace `bot_token`!")
            ...
            for (const file of commandFiles) {
                ...
                embed.addFields({name: `**${config.prefix}\`${command.help.name}${command.help.token ? ` bot_token` : ""}\`**`, value: `${config.emoji ?? ""} ${command.help.description ?? "Aucune description"}`})
            }
